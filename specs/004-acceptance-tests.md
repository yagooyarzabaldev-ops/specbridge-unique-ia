...
### AT-036 - Standard Loop v1 exists

Pass condition:

docs/specbridge-standard-loop-v1.md, docs/specbridge-standard-loop-feature-pilot.md, docs/specbridge-standard-templates.md, docs/specbridge-ci-authority-standard.md, and docs/specbridge-v5-live-parallel-pilot-boundary.md exist; scripts/specbridge.ps1 supports `standard-loop-status` and `execute-runtime-launch`; `standard-loop-status` reports templates, schemas, validators, existing GitHub CI workflow authority, latest artifacts, missing paths, and CI security boundary evidence; `execute-runtime-launch -DryRun` writes a validated `.specbridge/runtime-executions/*.runtime-execution.json` artifact without launching Claude Code or requiring network; templates/specbridge contains the standard contract, scope, handoff, runtime launch, final report, audit packet, and ChatGPT audit templates; schema validation includes executor packet, runtime launch, runtime run, runtime result, runtime summary, autonomy metrics, and runtime execution schemas; standard validation and smoke validation include template, CI authority, and runtime execution validation; CLI tests cover success and deterministic live execution rejection without `-Force`; negative tests cover invalid runtime execution, invalid standard template, and missing CI authority workflow evidence; local validations pass; GitHub CI passes before merge; and the task does not modify `.github/workflows/**`, touch protected credentials, production, billing, auth, database, dependency installation, hosted dashboard, MCP server, or deployment automation.
