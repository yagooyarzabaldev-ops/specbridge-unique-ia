...
- Lifecycle guard for apply-mode ordering (issue 153): in progress as lifecycle ordering correctness: post_merge_memory blocked unless primary PR state is MERGED, issue_close blocked unless merge_completed, merge returns auto_merge_enabled vs merge_completed distinct states, lifecycle-guard CLI command, specbridge-doctor lifecycle violations promoted to blockers, dashboard Open Lifecycle Debt section, policy.yaml path_overrides for specbridge-intake.yml.
