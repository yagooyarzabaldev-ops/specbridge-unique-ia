...
- Apply-mode merge expansion (issue 126): in progress, expanding apply-mode to support `gh pr merge --squash --auto` via the `merge` operation. Pilot scope guard updated to allow `issue_close`, `pr_open`, and `merge`. apply-unsupported-op test updated to use `ci_wait` operation. PR 129 pending CI.
