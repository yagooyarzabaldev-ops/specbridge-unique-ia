...
- Apply-mode merge expansion (issue 126): in progress, adding `gh pr merge --squash --auto` to apply mode. Pilot scope guard updated to allow `issue_close`, `pr_open`, and `merge`. PR 129 pending CI.
- Full end-to-end apply-mode loop test (issue 127): in progress, adding `issue-to-merge-github-full-loop-dry-run` CLI test verifying all 6 default operations produce the correct connector action envelope. PR pending CI.
