...
- ci_wait and post_merge_memory expansion (issue 140): in progress as the first governed expansion of the apply-mode pilot to include ci_wait (polling gh pr checks until CI passes) and post_merge_memory (automated closure branch creation, scope completion, and PR open after merge). Eliminates manual post-merge closure cycle. PR pending CI.
