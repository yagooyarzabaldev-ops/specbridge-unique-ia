...
- Apply-mode pr_open expansion (issue 123): in progress, expanding apply-mode to support `gh pr create` via the `pr_open` operation. Pilot scope guard updated to allow both `issue_close` and `pr_open`. apply-unsupported-op test updated to use `merge` operation. PR 124 pending CI.
