...
- Apply-mode issue_create operation (issue 142): in progress as the implementation of issue_create as the 6th apply-mode operation, completing the full autonomous 6-operation loop. issue_create uses create-or-verify semantics: searches for an existing GitHub issue by title; returns verified_existing if found; calls gh issue create if not. First live end-to-end run of all 6 operations: issue_create → pr_open → ci_wait → merge → issue_close → post_merge_memory.
