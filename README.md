...
- Apply-mode GitHub operator pilot (issue 119): in progress as the first real GitHub mutation — `issue-to-merge-github` apply mode now executes `gh issue close` when all evidence gates pass, using explicit `-Force -ConfirmGithubMutation -EvidencePath` confirmation and guarded by `apply_mode_pilot_supports_issue_close_only` scope.
