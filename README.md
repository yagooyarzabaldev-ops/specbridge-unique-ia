...
- Final standardization status (issue 252): `specbridge-final-standardization-status` reports the final standardization posture before serious product-build pilots, including 95 percent completion, 5 percent remaining policy-gated or pilot-gated work, categorized remaining gaps, blocked boundaries, recommended next contracts, validation expectations, evidence sources, and read-only/no-runtime/no-deployment boundaries.
...
- Agent SDK lightweight practices (issue 246): bounded MCP tools advertise `annotations.readOnlyHint: true`, Claude Code compaction instructions preserve governed task evidence, and token/context docs record future result/cost/session evidence fields without installing dependencies, hosting an SDK runtime, adding hooks, enabling session persistence, or expanding mutation authority.
...
- Governed branch cleanup (issue 249): complete through PR 250. A one-time exact authorization cleaned merged stale branch refs after Claude Code read-only classification review. The cleanup deleted 30 local stale branches, deleted 30 origin branches, removed 4 stale remote-tracking refs whose remote refs were already absent, and preserved `main`, the active issue branch until merge, the digital-twin branch without a merged PR, and the Tango research branch whose PR was closed without merge. General cleanup apply mode remains disabled.
